...
export function initNavigationBars() {
  ...
  document.getElementById("dock-dashboard-btn")?.addEventListener("click", () => {
    if (window.renderDashboardFromProfile) window.renderDashboardFromProfile();
    else showView("result-view");
  });
  document.getElementById("dock-history-btn")?.addEventListener("click", () => {
    const historyDetails = document.getElementById("history-details");
    if (historyDetails) {
      if (historyDetails.hasAttribute("open")) historyDetails.removeAttribute("open");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (backBtn) {
      ...
      if (subPane && paneMain) {
        ...
        paneMain.classList.remove("opacity-0", "pointer-events-none");
        paneMain.classList.add("opacity-100", "pointer-events-auto");
        setTimeout(() => { subPane.classList.add("hidden"); }, 300);
      }
    }
  });
...
export function updateNetworkStatus() {
  ...
  if (overlay) {
    ...
    else { overlay.classList.remove("hidden"); stopScanner(); }
  }

  if (checkerBadge) {
    if (isOnline) {
      checkerBadge.innerText = "Checker Active";
      checkerBadge.className = "text-[10px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md font-bold uppercase";
    } else {
      checkerBadge.innerText = "Cached View";
      checkerBadge.className = "text-[10px] text-gray-50 bg-gray-500 px-2 py-0.5 rounded-md font-bold uppercase";
    }
  }

  if (startScanBtn) { startScanBtn.disabled = !isOnline; startScanBtn.classList.toggle("opacity-50", !isOnline); }
  if (submitUrlBtn) { submitUrlBtn.disabled = !isOnline; submitUrlBtn.classList.toggle("opacity-50", !isOnline); }
  if (manualInput) manualInput.disabled = !isOnline;
  if (openOriginalBtn) { openOriginalBtn.disabled = !isOnline; openOriginalBtn.classList.toggle("opacity-50", !isOnline); }
}
...
export function showView(viewId) {
  ...
  if (historyWrapper) {
    if (viewId === "scanner-view") historyWrapper.classList.remove("hidden");
    else historyWrapper.classList.add("hidden");
  }
...
export function switchResultTab(tabName = 'overview') {
  ...
  tabs.forEach(t => {
    ...
    if (btn) btn.className = (t === activeTab) ? activeClass : inactiveClass;
    if (pane) {
      if (t === activeTab) pane.classList.remove('hidden');
